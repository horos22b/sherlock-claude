...
sub print_page {
    ...
    foreach my $line (@lines) {
        ...
        } elsif ($in_subpage_section && $line !~ /^[A-Z]:/) {
            # Continue adding lines within the current subpage section
            if ($subpages_printed > 0 && $subpages_printed <= $visible_subpages) {
                push @subpage_content, $line;
            }
        } else {
            $in_subpage_section = 0;
            push @subpage_content, $line;
        }
    }
...
I also highly recommend the youtube channel https://www.youtube.com/@COOPFORTWO for a good
introduction.</y>
